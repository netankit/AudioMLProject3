...
def getMfccVector(noise_mix_speech_file):
	...
	mfcc_vec = mfcc(signal,rate,winlen=0.025,winstep=0.02,numcep=mfcc_dimensions,
...
def getDataset(audio_files_dir):
	...
	for root, dirs, files in os.walk(audio_files_dir):
		...
		for file in files:
			...
			if (file.lower().endswith('.wav') and str(file)=="mix_out.wav"):
				...
				mfcc_vector_filename= 'mfcc_'+str(root_dir_name)+'.dat'

				mfcc_vector_output_file = os.path.join(mfcc_vector_output_filedir,mfcc_vector_filename)
